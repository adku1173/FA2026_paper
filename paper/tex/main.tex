...
Let $\ell \geq \|A^\mathsf{T}A\|_2$ be a Lipschitz constant of $\nabla f(x)=A^\mathsf{T}(Ax-y)$.
...
\begin{align}
    ...
    \bigl|a_n^\mathsf{T}(Ax^{(r)}-y)\bigr| > (1-\tau)\lambda\bigr\},
\end{align}
...
\begin{split}
    x_{\Omega^{(r)}}^{(r+1)} &=
    \arg\min_{u \geq 0} \;
    \tfrac{1}{2}\|A_{\Omega^{(r)}}u-y\|_2^2 + \lambda\|u\|_1, \\
    x_{(\Omega^{(r)})^c}^{(r+1)} &= 0,
\end{split}
\end{equation}
using a Lipschitz constant $\ell^{(r)} \geq \|A_{\Omega^{(r)}}^\mathsf{T}A_{\Omega^{(r)}}\|_2$ estimated by backtracking.
